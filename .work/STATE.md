...
- **Architectural State (includes Attempt 8 changes)**:
  ...
  3. **Header**: `backdrop-blur-md` removed → solid `bg-[#060402]/95` (Chromium backdrop-filter uninitialized-white path eliminated).
  4. **Typography & UI**: Outfit + Tajawal + IBM Plex Sans Arabic, 3-column academic stages, minimalist experience vertical stepper, all functioning properly.
...
- **Next Priority**: Deploy `dist/` to GitHub Pages and have the user hard-refresh repeatedly. If white persists, run the layer-inspector trace during the white state (see HANDOFF.md §5). If confirmed gone, mark this bug RESOLVED and update HANDOFF.md/STATE.md/MEMORY.md.
